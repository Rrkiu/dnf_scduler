...
export default function PartySlot({
  ...
  return (
    <div
      className={`border-t-4 ${color.borderTop} ${color.bg} border border-gray-200 rounded-lg p-3 flex flex-col items-center min-h-[120px] hover:shadow-sm transition`}
    >
      ...
      {isLoading ? (
        <div className="text-sm text-gray-400 py-2">Updating...</div>
      ) : isUnassignedColumn ? (
        <div className="flex-1 flex items-center justify-center">
          <span className="text-xs text-gray-400 italic text-center">모험단 미배정</span>
        </div>
      ) : (
        <select
          className={`w-full border border-gray-200 rounded p-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-gray-300 ${
            !character_id ? 'text-gray-400' : 'text-gray-900 font-medium'
          }`}
          value={character_id || ''}
      ...
      <div className="mt-2 text-xs w-full text-center min-h-[36px] flex flex-col items-center justify-center gap-1">
        {assignedChar ? (
          <>
            <span className="font-semibold text-gray-700">{assignedChar.job}</span>
            <span
